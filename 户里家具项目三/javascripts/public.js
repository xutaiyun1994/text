...
function changeAIdots() {
    for(var i = 0; i < aIdot.length; i++) {
        if(aIdot[i].classList.contains('active')) {
            aIdot[i].classList.remove('active');
            break;
        }
    }
    aIdot[curImgIdx].classList.add('active');
}
